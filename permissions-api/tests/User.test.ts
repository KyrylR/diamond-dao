import { assert, afterEach, clearStore, describe, newMockEvent, test } from "matchstick-as";
...
describe("User", () => {
  ...
  test("should handle TokensDeposited event", () => {
    let tokenAddress = Address.fromString("0x86e08f7d84603AEb97cd1c89A80A9e914f181679");
    let sender = Address.fromString("0x86e08f7d84603AEb97cd1c89A80A9e914f181678");
    ...
    assert.fieldEquals("User", sender.toHexString(), "id", sender.toHexString());
    assert.fieldEquals("User", sender.toHexString(), "balances", "[1]");
    assert.fieldEquals("User", sender.toHexString(), "tokens", `[${tokenAddress.toHexString()}]`);
  });
  ...
  test("should handle TokensWithdrew event", () => {
    ...
    onDeposit(createDepositEvent(tokenAddress, sender, BigInt.fromI32(10), block, tx));

    onWithdraw(createWithdrawEvent(tokenAddress, sender, BigInt.fromI32(4), block, tx));
