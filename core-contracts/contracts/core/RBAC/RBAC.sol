 ...
 *
 * More info: https://github.com/dl-solidity-library/dev-modules/blob/master/contracts/access-control/RBAC.sol
 */
abstract contract RBAC is IRBAC {
    ...
    bytes32 public constant RBAC_STORAGE_SLOT = keccak256("diamond.standard.rbac.storage");

    struct RBACStorage {
        bool initialized;
        mapping(string => mapping(bool => mapping(string => StringSet.Set))) rolePermissions;
        mapping(string => mapping(bool => StringSet.Set)) roleResources;
        mapping(address => StringSet.Set) userRoles;
    }
    ...
    function getRBACStorage() internal pure returns (RBACStorage storage rbacStorage_) {
        bytes32 slot_ = RBAC_STORAGE_SLOT;

        assembly {
            rbacStorage_.slot := slot_
        }
    }
     ...
     */
    function __RBAC_init() internal {
        RBACStorage storage rbacStorage_ = getRBACStorage();

        require(!rbacStorage_.initialized, "RBAC: already initialized");

        _addPermissionsToRole(MASTER_ROLE, ALL_RESOURCE, ALL_PERMISSION.asSingletonArray(), true);

        rbacStorage_.initialized = true;
    }
    ...
    function getUserRoles(address who_) public view override returns (string[] memory roles_) {
        return getRBACStorage().userRoles[who_].values();
    }
        ...
        )
    {
        ...
        StringSet.Set storage _allowedResources = rbacStorage_.roleResources[role_][true];
        StringSet.Set storage _disallowedResources = rbacStorage_.roleResources[role_][false];

        mapping(string => StringSet.Set) storage _allowedPermissions = rbacStorage_
            .rolePermissions[role_][true];
        mapping(string => StringSet.Set) storage _disallowedPermissions = rbacStorage_
            .rolePermissions[role_][false];
    ...
    function _grantRoles(address to_, string[] memory rolesToGrant_) internal {
        getRBACStorage().userRoles[to_].add(rolesToGrant_);
    ...
    function _revokeRoles(address from_, string[] memory rolesToRevoke_) internal {
        getRBACStorage().userRoles[from_].remove(rolesToRevoke_);
    ...
    function _addPermissionsToRole(
        ...
        StringSet.Set storage _resources = rbacStorage_.roleResources[role_][allowed_];
        StringSet.Set storage _permissions = rbacStorage_.rolePermissions[role_][allowed_][
            resourceToAdd_
        ];
    ...
    function _isAllowed(
        ...
        mapping(string => StringSet.Set) storage _resources = getRBACStorage().rolePermissions[
            role_
        ][true];
